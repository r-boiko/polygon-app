...
const DailyPriceComponent = ({ symbol }: Props) => {
  const { lastAvailablePrice, priceDifference, changePercent } = useTickerDailyPrice(symbol);
  ...
  return lastAvailablePrice && priceDifference && changePercent
    ? (
      <div className={`${styles.headerInfoContainer} ${styles.headerInfoContainerMobile}`}>
        <div className={styles.priceTitle}>${lastAvailablePrice}</div>
        <div className={styles.priceIndicatorWrapper}>
        <span className={isPositiveNumber ? styles.pricePositive : styles.priceLow}>
          {priceDifference}
        </span>
